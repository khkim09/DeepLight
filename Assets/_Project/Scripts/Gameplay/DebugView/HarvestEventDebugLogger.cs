...
namespace Project.Gameplay.DebugView
{
    ...
    public class HarvestEventDebugLogger : MonoBehaviour
    {
        ...
        {
            Debug.Log($"[Mode] {publishedEvent.PreviousMode} -> {publishedEvent.CurrentMode}");
        }

        /// <summary>세션 시작 로그를 출력</summary>
        private void OnHarvestSessionStarted(HarvestSessionStartedEvent publishedEvent)
        {
            Debug.Log($"[Harvest Session Started] Target={publishedEvent.TargetKey}, Item={publishedEvent.ItemId}");
        }

        /// <summary>세션 종료 로그를 출력</summary>
        private void OnHarvestSessionEnded(HarvestSessionEndedEvent publishedEvent)
        {
            Debug.Log($"[Harvest Session Ended] Target={publishedEvent.TargetKey}");
        }
        ...
        {
            Debug.Log($"[Harvest Session Forced End] Target={publishedEvent.TargetKey}");
        }

        /// <summary>시도 시작 로그를 출력</summary>
        private void OnHarvestAttemptStarted(HarvestAttemptStartedEvent publishedEvent)
        {
            Debug.Log($"[Harvest Attempt Started] Item={publishedEvent.ItemId}, Preview={(publishedEvent.PreviewChance * 100f):0.0}%");
        }

        /// <summary>시도 결과 로그를 출력</summary>
        private void OnHarvestAttemptResolved(HarvestAttemptResolvedEvent publishedEvent)
        {
            Debug.Log($"[Harvest Attempt Resolved] Item={publishedEvent.ItemId}, Success={publishedEvent.IsSuccess}, Added={publishedEvent.AddedToInventory}, Chance={(publishedEvent.FinalChance * 100f):0.0}%");
        }

        /// <summary>배터리 로그를 출력</summary>
        private void OnBatteryChanged(BatteryChangedEvent publishedEvent)
        {
            Debug.Log($"[Battery] {publishedEvent.CurrentBattery:0.0}/{publishedEvent.MaxBattery:0.0}");
        }

        /// <summary>인벤토리 추가 로그를 출력</summary>
        private void OnInventoryItemAdded(InventoryItemAddedEvent publishedEvent)
        {
            Debug.Log($"[Inventory Added] Item={publishedEvent.ItemId}, Amount={publishedEvent.Amount}");
        }
    }
}
