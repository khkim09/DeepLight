...
namespace Project.Editor.AutoTool
{
    ...
    public static class WorldMapAssetHelper
    {
        ...
        public const string ENVIRONMENT_FOLDER = "Assets/_Project/ScriptableObjects/World/EnvironmentProfiles";
        public const string SCENARIO_FOLDER = "Assets/_Project/ScriptableObjects/World/ScenarioPresets";

        private static readonly string[] AllFolders = { CONFIG_FOLDER, ZONES_FOLDER, UNLOCK_CONDITIONS_FOLDER, DEBUG_FOLDER, AMBIENT_FOLDER, ENVIRONMENT_FOLDER, SCENARIO_FOLDER };
        ...
        public static void CreateUpdateAllAssets()
        {
            ...
            CreateAllEnvironmentProfiles();
            CreateAllScenarioPresets();
        ...
        /// <summary>4개의 ZoneEnvironmentProfileSO 에셋을 생성/보정한다.</summary>
        public static void CreateAllEnvironmentProfiles()
        {
            CreateFolder(ENVIRONMENT_FOLDER);

            CreateOrPatchEnvironmentProfile("ZoneEnvProfile_Hub_Safe", (so) =>
            {
                so.FindProperty("depthBand").enumValueIndex = (int)ZoneDepthBand.Surface;
                ...
                so.FindProperty("debugColor").colorValue = new Color(0.2f, 0.8f, 0.6f, 1f);
            });

            CreateOrPatchEnvironmentProfile("ZoneEnvProfile_Research_Tense", (so) =>
            {
                ...
                so.FindProperty("maxDepth").floatValue = -300f;
                ...
                so.FindProperty("ruinDensity").floatValue = 0.2f;
                so.FindProperty("hazardDensity").floatValue = 0.25f;
                so.FindProperty("debugColor").colorValue = new Color(0.8f, 0.6f, 0.1f, 1f);
            });

            CreateOrPatchEnvironmentProfile("ZoneEnvProfile_Sealed_Danger", (so) =>
            {
                ...
                so.FindProperty("maxDepth").floatValue = -800f;
                so.FindProperty("basePropDensity").floatValue = 0.5f;
                so.FindProperty("rockDensity").floatValue = 0.4f;
                ...
                so.FindProperty("debugColor").colorValue = new Color(0.8f, 0.1f, 0.1f, 1f);
            });

            CreateOrPatchEnvironmentProfile("ZoneEnvProfile_OutOfBounds_Default", (so) =>
            {
                ...
                so.FindProperty("waterLevelY").floatValue = 0f;
                so.FindProperty("minDepth").floatValue = 0f;
                so.FindProperty("maxDepth").floatValue = -2000f;
                so.FindProperty("basePropDensity").floatValue = 0f;
                so.FindProperty("rockDensity").floatValue = 0f;
                so.FindProperty("vegetationDensity").floatValue = 0f;
                so.FindProperty("wreckDensity").floatValue = 0f;
                so.FindProperty("ruinDensity").floatValue = 0f;
                so.FindProperty("hazardDensity").floatValue = 0f;
                so.FindProperty("debugColor").colorValue = new Color(0.3f, 0.3f, 0.3f, 1f);
            });

            LogSummary("[EnvironmentProfile] 4개 프로필 생성 완료.");
        }

        private static void CreateOrPatchEnvironmentProfile(string assetName, System.Action<SerializedObject> patchAction)
        {
            string path = $"{ENVIRONMENT_FOLDER}/{assetName}.asset";
            var existing = AssetDatabase.LoadAssetAtPath<ZoneEnvironmentProfileSO>(path);
            if (existing != null)
            {
                LogFound($"EnvironmentProfile (이미 존재): {assetName}");
                SerializedObject so = new SerializedObject(existing);
                patchAction(so);
                so.ApplyModifiedProperties();
        ...
        /// <summary>기본 WorldMapScenarioPresetSO 에셋을 생성/보정한다. zoneRules를 기획서 기준으로 채운다.</summary>
        public static void CreateAllScenarioPresets()
        {
            CreateFolder(SCENARIO_FOLDER);

            // EnvironmentProfile 참조 로드 (없으면 null, 생성은 WorldMapScenarioPresetCreator에서 담당)
            var envHub = AssetDatabase.LoadAssetAtPath<ZoneEnvironmentProfileSO>($"{ENVIRONMENT_FOLDER}/ZEP_Hub.asset");
            var envShallowWreck = AssetDatabase.LoadAssetAtPath<ZoneEnvironmentProfileSO>($"{ENVIRONMENT_FOLDER}/ZEP_ShallowWreck.asset");
            var envResearchField = AssetDatabase.LoadAssetAtPath<ZoneEnvironmentProfileSO>($"{ENVIRONMENT_FOLDER}/ZEP_ResearchField.asset");
            var envSealedNorth = AssetDatabase.LoadAssetAtPath<ZoneEnvironmentProfileSO>($"{ENVIRONMENT_FOLDER}/ZEP_SealedNorth.asset");
            var envOuterSea = AssetDatabase.LoadAssetAtPath<ZoneEnvironmentProfileSO>($"{ENVIRONMENT_FOLDER}/ZEP_OuterSea.asset");
            var envCanyon = AssetDatabase.LoadAssetAtPath<ZoneEnvironmentProfileSO>($"{ENVIRONMENT_FOLDER}/ZEP_Canyon.asset");
            var envAbyssExperiment = AssetDatabase.LoadAssetAtPath<ZoneEnvironmentProfileSO>($"{ENVIRONMENT_FOLDER}/ZEP_AbyssExperiment.asset");
            var envOpenWater = AssetDatabase.LoadAssetAtPath<ZoneEnvironmentProfileSO>($"{ENVIRONMENT_FOLDER}/ZEP_OpenWater.asset");

            CreateOrPatchScenarioPreset("WorldMapScenario_Default", (so) =>
            {
                // zoneRules 배열 설정 (비우지 않고 기획서 기준으로 채움)
                var rulesProp = so.FindProperty("zoneRules");
                rulesProp.ClearArray();

                // Rule 1: 중앙 허브권 (E5, F5, E6, F6)
                AddScenarioZoneRule(rulesProp, "Hub Zone", new string[] { "E5", "F5", "E6", "F6" }, "Hub",
                    ZoneDepthBand.Shallow, ZoneBiomeType.Hub, 0f, -120f, 0.05f, envHub,
                    new Color(0.3f, 0.9f, 0.5f, 1f));

                // Rule 2: 서쪽 폐선권 (B5, B6, C5, C6, C7)
                AddScenarioZoneRule(rulesProp, "Shallow Wreck Zone", new string[] { "B5", "B6", "C5", "C6", "C7" }, "WestWreck",
                ...
                AddScenarioZoneRule(rulesProp, "Research Field Zone", new string[] { "H5", "H6", "I5", "I6", "I7" }, "EastResearch",
                ...
                AddScenarioZoneRule(rulesProp, "Sealed North Zone", new string[] { "E9", "F9", "E10", "F10", "G9", "G10" }, "SealedNorth",
                ...
                AddScenarioZoneRule(rulesProp, "Outer Sea Zone", new string[] { "E1", "F1", "E2", "F2", "G1" }, "OuterSea",
                ...
                AddScenarioZoneRule(rulesProp, "Canyon Zone", new string[] { "A8", "A9", "B8", "B9" }, "Canyon",
                ...
                AddScenarioZoneRule(rulesProp, "Abyss Experiment Zone", new string[] { "I9", "J9", "I10", "J10" }, "AbyssExperiment",
            ...
            LogSummary("[ScenarioPreset] 1개 프리셋 생성 완료 (7개 Zone Rule 포함).");
        }
        ...
        /// </summary>
        private static void AddScenarioZoneRule(
        ...
        private static void CreateOrPatchScenarioPreset(string assetName, System.Action<SerializedObject> patchAction)
        {
            string path = $"{SCENARIO_FOLDER}/{assetName}.asset";
            var existing = AssetDatabase.LoadAssetAtPath<WorldMapScenarioPresetSO>(path);
            if (existing != null)
            {
                LogFound($"ScenarioPreset (이미 존재): {assetName}");
                SerializedObject so = new SerializedObject(existing);
                patchAction(so);
                so.ApplyModifiedProperties();
