...
{
    /// <summary>
    /// WorldMapRuntimeFinalContentInstanceRegistry에 대한 얇은 Query Facade.
    ...
    /// </summary>
    public class WorldMapRuntimeFinalContentQueryService : MonoBehaviour
    {
        ...
        [SerializeField, Tooltip("실제 cache를 보유한 Final Content Instance Registry")]
        private WorldMapRuntimeFinalContentInstanceRegistry registry;
        ...
        /// <summary>Registry에 cache된 final content instance 총 개수</summary>
        ...
        private void Awake()
        {
            if (initializeOnAwake && !hasRuntimeInitialized)
            {
                if (logRuntimeInitialization)
                    UnityEngine.Debug.Log("[WorldMapRuntimeFinalContentQueryService] Awake: initializeOnAwake is true. Initializing from root.");
        ...
        private void OnEnable()
        {
            ...
            {
                if (logRuntimeInitialization)
                    UnityEngine.Debug.Log("[WorldMapRuntimeFinalContentQueryService] OnEnable: Count is 0. Initializing from root.");
        ...
        /// GeneratedWorldRoot에 WorldMapRuntimeFinalContentInstanceRegistry가 없으면
        ...
        public bool TryInitializeFromRoot(Transform generatedRoot)
        {
            ...
            {
                UnityEngine.Debug.LogWarning("[WorldMapRuntimeFinalContentQueryService] TryInitializeFromRoot: generatedRoot is null.");
                return false;
            }

            // GeneratedWorldRoot에서 Registry component 검색
            WorldMapRuntimeFinalContentInstanceRegistry foundRegistry = generatedRoot.GetComponent<WorldMapRuntimeFinalContentInstanceRegistry>();
            if (foundRegistry == null)
            {
                // Registry가 없으면 같은 root GameObject에 추가
                foundRegistry = generatedRoot.gameObject.AddComponent<WorldMapRuntimeFinalContentInstanceRegistry>();
                UnityEngine.Debug.Log($"[WorldMapRuntimeFinalContentQueryService] Added WorldMapRuntimeFinalContentInstanceRegistry to '{generatedRoot.name}'.");
            }
            ...
            {
                if (logRuntimeInitialization)
                    UnityEngine.Debug.Log("[WorldMapRuntimeFinalContentQueryService] Registry cache is empty. Calling RebuildCacheFromRoot.");

                registry.RebuildCacheFromRoot(generatedRoot);
            }

            UnityEngine.Debug.Log($"[WorldMapRuntimeFinalContentQueryService] Initialized from '{generatedRoot.name}'. Registry final content instances: {registry.Count}");
            return true;
        }
        ...
        public bool Rebuild()
        {
            if (registry == null)
            {
                UnityEngine.Debug.LogWarning("[WorldMapRuntimeFinalContentQueryService] Rebuild skipped: registry is null.");
                return false;
            }
            ...
            UnityEngine.Debug.Log($"[WorldMapRuntimeFinalContentQueryService] Rebuild complete. Registry final content instances: {registry.Count}");
            return true;
        }
        ...
        /// <summary>
        /// 모든 final content instance의 읽기 전용 목록을 반환한다.
        /// registry가 null이면 빈 목록을 반환한다.
        /// </summary>
        public IReadOnlyList<WorldMapRuntimeFinalContentInstanceTag> GetAll()
        {
            if (registry == null)
        ...
        public IReadOnlyList<WorldMapRuntimeFinalContentInstanceTag> GetByFinalContentKind(WorldMapRuntimeFinalContentKind finalContentKind)
        {
            ...
            return registry.GetByFinalContentKind(finalContentKind);
        }
        ...
        /// <param name="result">찾은 final content instance, 없으면 null</param>
        ...
        /// <summary>
        /// 지정한 worldPosition에서 가장 가까운 final content instance를 찾는다.
        /// registry가 null이면 false를 반환한다.
        /// FinalContentKind.None이 들어오면 전체 대상에서 nearest를 찾는다.
        /// </summary>
        /// <param name="worldPosition">검색 기준 월드 좌표</param>
        /// <param name="finalContentKind">필터링할 FinalContentKind (None이면 전체 대상)</param>
        /// <param name="result">가장 가까운 final content instance, 없으면 null</param>
        /// <returns>찾은 경우 true, 없으면 false</returns>
        public bool TryGetNearest(Vector3 worldPosition, WorldMapRuntimeFinalContentKind finalContentKind, out WorldMapRuntimeFinalContentInstanceTag result)
        {
            result = null;
            ...
            IReadOnlyList<WorldMapRuntimeFinalContentInstanceTag> candidates;
            ...
            WorldMapRuntimeFinalContentInstanceTag nearest = null;

            foreach (WorldMapRuntimeFinalContentInstanceTag candidate in candidates)
            {
        ...
        /// <summary>
        /// 지정한 worldPosition에서 가장 가까운 final content instance를 RuntimeCategory 기준으로 찾는다.
        ...
        /// <param name="result">가장 가까운 final content instance, 없으면 null</param>
        /// <returns>찾은 경우 true, 없으면 false</returns>
        public bool TryGetNearest(Vector3 worldPosition, string runtimeCategory, out WorldMapRuntimeFinalContentInstanceTag result)
        {
            result = null;
        ...
        /// <summary>
        /// 지정한 Zone 내에서 worldPosition에 가장 가까운 final content instance를 찾는다.
        /// registry가 null이거나 zoneId가 null/empty이면 false를 반환한다.
        /// FinalContentKind.None이 들어오면 해당 Zone 전체 대상에서 nearest를 찾는다.
        /// </summary>
        /// <param name="zoneId">검색 범위가 되는 ZoneId</param>
        /// <param name="worldPosition">검색 기준 월드 좌표</param>
        /// <param name="finalContentKind">필터링할 FinalContentKind (None이면 Zone 전체 대상)</param>
        /// <param name="result">가장 가까운 final content instance, 없으면 null</param>
        /// <returns>찾은 경우 true, 없으면 false</returns>
        public bool TryGetNearestInZone(string zoneId, Vector3 worldPosition, WorldMapRuntimeFinalContentKind finalContentKind, out WorldMapRuntimeFinalContentInstanceTag result)
        {
            result = null;
            ...
            // ZoneId로 1차 필터링
            IReadOnlyList<WorldMapRuntimeFinalContentInstanceTag> zoneCandidates = registry.GetByZoneId(zoneId);
            if (zoneCandidates == null || zoneCandidates.Count == 0)
                return false;

            // FinalContentKind로 2차 필터링 (None이 아닌 경우)
            List<WorldMapRuntimeFinalContentInstanceTag> candidates = null;
            if (finalContentKind != WorldMapRuntimeFinalContentKind.None)
            {
                candidates = new List<WorldMapRuntimeFinalContentInstanceTag>(zoneCandidates.Count);
                foreach (WorldMapRuntimeFinalContentInstanceTag candidate in zoneCandidates)
                {
                    if (candidate != null && candidate.FinalContentKind == finalContentKind)
                    {
            ...
            else
            {
                ...
                candidates = new List<WorldMapRuntimeFinalContentInstanceTag>(zoneCandidates);
            }
        ...
        public int CountByFinalContentKind(WorldMapRuntimeFinalContentKind finalContentKind)
        {
            ...
            return registry.CountByFinalContentKind(finalContentKind);
        }
