...
{
    /// <summary>
    /// Harvest 시스템이 WorldMapRuntimeHarvestInteractionCandidateRegistry를 직접 만지지 않도록
    ...
    /// </summary>
    public class WorldMapRuntimeHarvestInteractionCandidateQueryService : MonoBehaviour
    {
        // ===== Serialized Fields =====
        ...
        private WorldMapRuntimeHarvestInteractionCandidateRegistry _registry;
        ...
        /// <summary>QueryService가 관리하는 interaction candidate 총 개수</summary>
        ...
        private void Awake()
        {
            if (initializeOnAwake && !_isInitialized)
            {
                if (logRuntimeInitialization)
                    UnityEngine.Debug.Log("[WorldMapRuntimeHarvestInteractionCandidateQueryService] Awake: initializeOnAwake is true. Initializing from root.");
        ...
        private void OnEnable()
        {
            ...
            {
                if (logRuntimeInitialization)
                    UnityEngine.Debug.Log("[WorldMapRuntimeHarvestInteractionCandidateQueryService] OnEnable: registry is null or empty. Initializing from root.");
        ...
        public bool TryInitializeFromRoot(Transform root)
        {
            ...
            if (_registry == null)
            {
                _registry = root.GetComponentInChildren<WorldMapRuntimeHarvestInteractionCandidateRegistry>(true);

                if (_registry == null)
                {
                    // root.gameObject에 Registry 추가
                    _registry = root.gameObject.AddComponent<WorldMapRuntimeHarvestInteractionCandidateRegistry>();
                    if (logRuntimeInitialization)
                        UnityEngine.Debug.Log("[WorldMapRuntimeHarvestInteractionCandidateQueryService] Added WorldMapRuntimeHarvestInteractionCandidateRegistry to root.");
                }
            }
        ...
        public bool Rebuild()
        {
            ...
            {
                UnityEngine.Debug.LogWarning("[WorldMapRuntimeHarvestInteractionCandidateQueryService] Rebuild: registry is null. Call TryInitializeFromRoot first.");
                return false;
            }
        }

        /// <summary>
        /// 모든 interaction candidate를 반환한다.
        /// </summary>
        public IReadOnlyList<WorldMapRuntimeHarvestInteractionCandidate> GetAll()
        {
            if (_registry == null)
        ...
        /// <summary>
        /// 지정한 position에서 가장 가까운 interaction candidate를 찾는다.
        /// </summary>
        /// <param name="position">기준 위치</param>
        /// <param name="result">가장 가까운 candidate (없으면 null)</param>
        /// <returns>찾은 경우 true, 없으면 false</returns>
        public bool TryGetNearest(Vector3 position, out WorldMapRuntimeHarvestInteractionCandidate result)
        {
            result = null;
            ...
            WorldMapRuntimeHarvestInteractionCandidate nearest = null;
        ...
        /// <summary>
        /// 지정한 position에서 가장 가까운, 지정한 runtimeKey를 가진 interaction candidate를 찾는다.
        /// </summary>
        /// <param name="position">기준 위치</param>
        /// <param name="runtimeKey">필터링할 RuntimeKey</param>
        /// <param name="result">가장 가까운 candidate (없으면 null)</param>
        /// <returns>찾은 경우 true, 없으면 false</returns>
        public bool TryGetNearest(Vector3 position, string runtimeKey, out WorldMapRuntimeHarvestInteractionCandidate result)
        {
            result = null;
        ...
        /// <summary>
        /// 지정한 position에서 가장 가까운, 지정한 InteractionKind를 가진 interaction candidate를 찾는다.
        /// </summary>
        /// <param name="position">기준 위치</param>
        /// <param name="kind">필터링할 InteractionKind</param>
        /// <param name="result">가장 가까운 candidate (없으면 null)</param>
        /// <returns>찾은 경우 true, 없으면 false</returns>
        public bool TryGetNearest(Vector3 position, WorldMapRuntimeHarvestInteractionCandidateKind kind, out WorldMapRuntimeHarvestInteractionCandidate result)
        {
            result = null;
            ...
            foreach (var candidate in _registry.AllCandidates)
            {
                if (!candidate.IsInteractionKind(kind))
                    continue;
        ...
        /// <summary>
        /// 지정한 ZoneId 내에서 position에 가장 가까운 interaction candidate를 찾는다.
        /// </summary>
        /// <param name="zoneId">필터링할 ZoneId</param>
        /// <param name="position">기준 위치</param>
        /// <param name="result">가장 가까운 candidate (없으면 null)</param>
        /// <returns>찾은 경우 true, 없으면 false</returns>
        public bool TryGetNearestInZone(string zoneId, Vector3 position, out WorldMapRuntimeHarvestInteractionCandidate result)
        {
            result = null;
