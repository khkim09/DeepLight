...
namespace StylizedWater3.DynamicEffects
{
    ...
    public class ShorelineWaveSpawner : MonoBehaviour
    {
        ...
        {
            if (audioSource.transform == this.transform)
            {
                throw new Exception($"Audio source transform ({audioSource.name}) must on a separate Transform. Using the same transform as this component causes the spawned waves to also move");
            }
            if (!audioListener) audioListener = FindAnyObjectByType<AudioListener>();
