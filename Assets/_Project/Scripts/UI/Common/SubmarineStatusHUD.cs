...
{
    /// <summary>배터리와 내구도 상태를 공통 HUD에 표시하는 클래스이다.</summary>
    public class SubmarineStatusHUD : MonoBehaviour
    {
        ...
        [SerializeField] private TMP_Text durabilityText; // 내구도 텍스트
        ...
        private void OnBatteryChanged(BatteryChangedEvent publishedEvent)
        {
            ...
            if (batteryText != null)
                batteryText.text = $"{publishedEvent.CurrentBattery:0}/{publishedEvent.MaxBattery:0}";
        }
        ...
        private void OnHullDurabilityChanged(HullDurabilityChangedEvent publishedEvent)
        {
            ...
            if (durabilityText != null)
                durabilityText.text = $"{publishedEvent.CurrentDurability:0}/{publishedEvent.MaxDurability:0}";
        }
    }
}
