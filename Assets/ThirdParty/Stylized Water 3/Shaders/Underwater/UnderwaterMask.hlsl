...
UnderwaterMaskVaryings VertexWaterLine(UnderwaterMaskAttributes input)
{
	...
	input.positionOS.x *= 1.25;
	#if _WATERLINE
	input.positionOS.y *= 1.10;
	#endif
	#endif
	
	//Position vertices on the near-clip plane and scale to fit
	float3 basePosition = nearPlaneCenter + (CAM_RIGHT * (input.positionOS.x * aspectRatio * fovScalar) * clipPlane);
	
	float3 positionWS = basePosition + (CAM_UP * input.positionOS.y * clipPlane * fovScalar);
	float3 bottom = basePosition - (CAM_UP * clipPlane * fovScalar);
	float3 top = basePosition + (CAM_UP * clipPlane * fovScalar);
	...
	//if(HasHitWaterSurface(waterHeight))
	{
		...
		float angle = acos(upFactor);

		//Distance from center to water level along tangent (from known opposite length and angle)
		float hypotenuse = depth / cos(angle);
		
		// Delta is just the hypotenuse (length of offset vector equals hypotenuse)
		float delta = hypotenuse;

		if(bottom.y <= waterHeight)
		{
			#if _WATERLINE
			//Center on water line
			delta -= planeLength * 0.5;
			positionWS += (CAM_UP * delta);
			#else
			
			//Position the top row of vertices only
...
float SampleUnderwaterMask(float2 screenPos)
{
	...
	if(_UnderwaterRenderingEnabled)
	{
		...
		{
			screenPos = UnityStereoTransformScreenSpaceTex(screenPos);
